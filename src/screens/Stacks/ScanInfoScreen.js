...
const ScanInfoScreen = ({ route, navigation }) => {
  ...
  return (
    ...
    <View style={styles.container}>
      {item && (
        <>
          <Image source={{uri:item.image_url}} style={image_Size} />
          ...
          <View style={styles.font_B}>{Object
          .keys(item.nutrient_levels)
          .map( (nutrient) => <Text key={nutrient}>{nutrient}: {item.nutrient_levels[nutrient]}</Text>)
          }</View>
          <Text style={styles.font_P}>{item.data_sources}</Text>
          <TouchableOpacity style={styles.button_2}>
            <Text style={styles.font_H2} onPress={() => navigation.goBack()}>
              Back
            </Text>
          </TouchableOpacity>
        </>
